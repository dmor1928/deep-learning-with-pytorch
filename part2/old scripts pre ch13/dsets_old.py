...
import copy  # For copying candidateInfo_list in LunaDataset from cached data
...
def getCandidateInfoList(requireOnDisk_bool=True):  # Returns clean dictionary data, accounting for coord error
    ...
    with open('luna/candidates.csv', "r") as f:  # Append rest of (non-nodule) data from candidates.csv
...
class Ct:  # Load CT scan data of given uid
    def __init__(self, series_uid):
        ...
        mhd_path = glob.glob('luna/subset*/{}.mhd'.format(series_uid))[0]  #subset not specified, but will be unique
        ...
        # Convert and clean CT data
        ct_a = np.array(sitk.GetArrayFromImage(ct_mhd), dtype=np.float32)  #PyTorch likes float32
        ...
        self.hu_a = ct_a  # Values of CT_scan data (in HU units)
    ...
    def getRawCandidate(self, center_xyz, width_irc):
        ...
        """
        center_irc = xyz2irc(  # Translate spatial coordinate to voxel coordinate for indexing
            center_xyz,
            self.origin_xyz,
            self.vxSize_xyz,
            self.direction_a
        ...
        for axis, center_val in enumerate(center_irc):  # For each coordinate axis, 0 (I), 1 (R), 2 (C)
            ...
            end_ndx = int(round(center_val + width_irc[axis]))  # max index in chunk for the axis

            assert center_val >= 0 and center_val < self.hu_a.shape[axis], (  # Asserts center_val is valid
    ...
    def buildAnnotationMask(self, positiveInfo_list, threshold_hu=-700):
        ...
        boundingBox_a = np.zeros_like(self.hu_a, dtype=bool)  # all-false boolean array with same size as CT scan
        ...
        for candidateInfo_tup in positiveInfo_list:
            ...
            try:  # Expand radius until HU value on one of the vertices < threshold, then stops
                while(
                        self.hu_a[ci + index_radius, cr, cc] > threshold_hu and
                        self.hu_a[ci - index_radius, cr, cc] > threshold_hu
                ):  # Ends when there is a 1-voxel thick border of low-density tissue (on side closest to centre)
                    index_radius += 1
            ...
            try:
                while (
                        self.hu_a[ci, cr + row_radius, cc] > threshold_hu and
            ...
            try:
                while (
                        self.hu_a[ci, cr, cc + col_radius] > threshold_hu and
...
class Luna2dSegmentationDataset(Dataset):
    def __init__(self,
        ...
        for series_uid in self.series_list:
            ...
            if series_uid == '1.3.6.1.4.1.14519.5.2.1.6279.6001.100398138793540579077826395208':
                print("confirmed: it IS in series_list")
        ...
        self.candidateInfo_list = [candidate for candidate in self.candidateInfo_list
                                   if candidate.series_uid in series_set]
        # Filters out candidates from series that are not in the training/validation set

        self.pos_list = [candidate for candidate in self.candidateInfo_list if candidate.isNodule_bool]
    ...
    def getitem_fullSlice(self, series_uid, slice_ndx):
        ...
        for i, context_ndx in enumerate(range(start_ndx, end_ndx)):
            ...
            context_ndx = min(context_ndx, ct.hu_a.shape[0])
            ct_t[i] = torch.from_numpy(ct.hu_a[context_ndx].astype(np.float32))
        ct_t.clamp_(-1000, 1000)

        pos_t = torch.from_numpy(ct.positive_mask[slice_ndx]).unsqueese(0)
...
class TrainingLuna2dSegmentationDataset(Luna2dSegmentationDataset):
    ...
    def getitem_trainingCrop(self, candidateInfo_tup):
        ...
        """
        ct_a, pos_a, center_irc = getCtRawCandidate(  # 96x96x7 chunk around candidate centre
            candidateInfo_tup.series_uid,
            candidateInfo_tup.center_xyz,
        ...
        print("shaoe of ct_t: ", np.shape(ct_t))

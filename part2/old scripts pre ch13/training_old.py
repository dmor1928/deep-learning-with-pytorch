...
class LunaTrainingApp:
    ...
    def logMetrics(
        ...
        log.info(
            "E{} {:8} {loss/neg:.4f} loss, {correct/neg:-5.1f}% correct ({neg_correct:} of {neg_count:})"
            .format(epoch_ndx, mode_str + '_neg', neg_correct=neg_correct, neg_count=neg_count, **metrics_dict,)
        )

        log.info(
            "E{} {:8} {loss/pos:.4f} loss, {correct/pos:-5.1f}% correct ({pos_correct:} of {pos_count:})"
...
class SegmentationTrainingApp:
    ...
    def logImages(self, epoch_ndx, mode_str, dl):
        ...
        for series_ndx, series_uid in enumerate(images):
            ...
            for slice_ndx in range(6):
                ...
                writer = getattr(self, mode_str + 'writer')  # Add image_a to Tensorboard
                writer.add_image(
                ...
                if epoch_ndx == 1:  # Label image never changes, so only add image in first epoch.
                    ...
                    writer.add_image(  # Add label_image_a to Tensorboard
                        f'{mode_str}/{series_ndx}_label{slice_ndx}',
                        label_image_a,
                        self.totalTrainingSamples_count,
                        dataformats='HWC'  # Order of axes is Height, Width, Channel
                    )
    ...
    def logMetrics(self, epoch_ndx, mode_str, metrics_t):
        ...
        metrics_dict['pr/loss_all'] = sum_a[METRICS_LOSS_NDX].mean()  # Mean loss
        ...
        log.info(("E{} {:8} "  # Log/save the true pos, false neg and false pos
                  + "{loss/all:.4f} loss,"
                  + "{percent_all/tp:-5.1f}% true pos, {percent_all/fn:-5.1f}% false neg, {percent_all/fp:-9.f1}% false pos")

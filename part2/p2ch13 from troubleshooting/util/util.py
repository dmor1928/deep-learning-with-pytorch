...
def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):
    # Voxel to patient coords:
    # 1. Flip IRC to CRI, to match XYZ directions
    # 2. Scale indices with voxel sizes
    # 3. Matrix-multiply with directions matrix
    # 4. Add (x,y,z) origin offset to all points

    cri_a = np.array(coord_irc)[::-1]   # Reverses order and converts to numpy
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    coords_xyz = (direction_a @ (cri_a * vxSize_a)) + origin_a  # 2. 3. and 4.
...
def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):
    ...
    cri_a = ((coord_a - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a  # 4. 3. and 2.
    cri_a = np.round(cri_a)  # Round to integers (float type)
    return IrcTuple(int(cri_a[2]), int(cri_a[1]), int(cri_a[0]))  # 1. and convert to int

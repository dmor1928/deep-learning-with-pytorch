...
def findPositiveSamples(start_ndx=0, limit=100):
    ...
    print("positiveSample_list: ", positiveSample_list)
...
def showCandidate(series_uid, batch_ndx=None, **kwargs):
    ...
    print("ds.candidateInfo_list ", ds.candidateInfo_list)
    ...
    subplot.set_title('index {}'.format(int(center_irc[0])), fontsize=20)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(15)
    plt.imshow(ct.hu_a[int(center_irc[0])], clim=clim, cmap='gray')

    subplot = fig.add_subplot(len(group_list) + 2, 3, 2)
    subplot.set_title('row {}'.format(int(center_irc[1])), fontsize=20)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(15)
    ...
    subplot.set_title('col {}'.format(int(center_irc[2])), fontsize=20)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(15)
    ...
    for row, index_list in enumerate(group_list):
        for col, index in enumerate(index_list):
            ...
            subplot.set_title('slice {}'.format(index), fontsize=20)
            for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
                label.set_fontsize(15)
